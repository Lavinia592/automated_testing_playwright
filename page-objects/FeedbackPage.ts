...
export class FeedbackPage {
  ...
  async assertClearedForm() {
    await expect(this.nameInput).toBeEmpty()
    await expect(this.commentInput).toBeEmpty()
  }

  async assertSubmittedForm() {
    await expect(this.feedbackTitle).toBeVisible() //the form has been submitted and the user is on the feedback page
  }
}
